...
OUT = "templates/Application-Tracker-Pro-Blank.xlsx"
...
UNIS = [
 ("Example: Technical University of Munich","Germany","Munich","Computational Biology","Prof. First Last",8,7,"Fully funded (example)","Not contacted","https://www.tum.de"),
...
banner(uni, 10)
uni.cell(row=2, column=1, value="Reference table — one row per university / supervisor. Applications pull Country, Supervisor and Funding from here automatically. Row 4 is a filled example; replace it and add your own below.").font = f(9, italic=True, color=GRAY)
...
APPS = [
 ...
 ("Example: Technical University of Munich","PhD in Computational Biology","Computational Biology","PhD",date(2026,10,5),"In Progress","High",8,50,date(2026,9,15),"Example row — delete or overwrite; supervisor/country/funding fill in automatically from the University name."),
] + [("","","","","","","",None,None,"","") for _ in range(29)]
banner(app, 16)
app.cell(row=2, column=1, value="Main tracker — edit the white columns. Grey columns (Country, Supervisor, Funding, Days Until Deadline) are calculated automatically. Row 4 is a filled example; replace it and add your own below.").font = f(9, italic=True, color=GRAY)
...
for i, row in enumerate(APPS):
    ...
    vals = {1: i+1, 2: uname, 3: f'=IFERROR(INDEX({ucol("B")},MATCH(B{r},{UR},0)),"")', 4: prog, 5: field, 6: level,
            ...
            10: status, 11: prio, 12: fit, 13: f'=IFERROR(INDEX({ucol("H")},MATCH(B{r},{UR},0)),"")', 14: (docs/100 if docs is not None else None),
            15: last, 16: notes,
...
picks = ["Example: Technical University of Munich","","",""]
...
countries = ["All"] + sorted({u[1] for u in UNIS if u[1]})
